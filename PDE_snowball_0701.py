...
def PDE_BTC(number):
    ...
    today_v2 = np.datetime64(time.strftime('%Y-%m-%d', time.localtime(time.time())))


    if flag_to_use_current_time == 1:
        current_time = sheet['现在时间点'].values    # 现在时间点
    else:
        current_time=[]
        for i in range(len(product_name)):
            current_time.append(today_v2)
    ...
    def cal_greeks(a, days_after_issuance,down_bound_cond, up_bound_cond, inside_obs_cond, final_ki_cond,
        ...
        # 求greeks
        f1 = a._mesh.shift(20).copy()
        f2 = a._mesh.copy()
        f3 = a._mesh.shift(-20).copy()
        a.delta_mesh = (f1 - f2) / f2 / (20 * a._dx)
        a.delta_mesh = a.delta_mesh.iloc[1:-1, :]
        a.charm_mesh = (-a.delta_mesh + a.delta_mesh.shift(-1, axis=1)) / (a._dt)
        a.gamma_mesh = (f1 - 2 * f2 + f3) / f2 / f2 / ((40*a._dx) ** 2)  # -(f1-f3)/(2*a._dx)/f2/f2
    ...
    a.M = M
    a.S_min = S_min
    dx = np.log(100/S_min)/(int(M*3/5))
    S_max = np.exp((M-int(M*3/5))*dx)*100
    a._und = und  
...
# 对excel写入操作
    ...
    current_time_cal = datetime.datetime.strptime(str(pd.to_datetime(current_time[number]).date()), '%Y-%m-%d')
    current_time_cal = datetime.datetime.date(current_time_cal)

    for i in range(len(date_series)):
        if int(rrule.rrule(rrule.DAILY, dtstart=current_time_cal, until=date_series[i]).count() - 1) == -1:
            continue
        else:
            days_to_next_obs = int(rrule.rrule(rrule.DAILY, dtstart=current_time_cal, until=date_series[i]).count() - 1)
            break
    ...
    wb = vb.load_workbook("雪球簿记交易对冲参数_v5.xlsx")
    ws = wb["簿记系统【BTC】"]
    if flag_to_use_current_time == 0:
        cell_current_time = ws.cell(number +2, 18)
        cell_current_time.value = str(current_time[number])
    # 输出 距离下一观察日天数 至excel
